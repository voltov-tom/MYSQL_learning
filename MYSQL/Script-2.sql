...
-- таблица закладки, можно добавлять людей в закладки, медиа, фотоальбомы, комюнити 
DROP TABLE IF EXISTS bookmarks;
CREATE TABLE bookmarks(
	id SERIAL PRIMARY KEY,
    user_id BIGINT UNSIGNED NOT NULL,
    media_id BIGINT UNSIGNED NOT NULL,
    bookmark_user BIGINT UNSIGNED NOT NULL,
    bookmark_communities VARCHAR(150),
    created_at DATETIME DEFAULT NOW(),
    
 	FOREIGN KEY (user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (media_id) REFERENCES media(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (bookmark_user) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
    FOREIGN KEY (bookmark_communities) REFERENCES communities(name) ON UPDATE CASCADE ON DELETE CASCADE
);
